...
-- RGB variant for legacy Kindle Home versions whose PDOC cover path rejects
-- single-component grayscale JPEGs.
local ffi = require("ffi")

local input = assert(arg[1], "missing PDF input")
local output = assert(arg[2], "missing PPM output")
local target_w = tonumber(arg[3]) or 168
local target_h = tonumber(arg[4]) or 259
...
local ok, err = xpcall(function()
    ...
    matrix.e = (target_w - source_w * scale) / 2 - bounds.x0 * scale
    matrix.f = (target_h - source_h * scale) / 2 - bounds.y0 * scale

    local byte_count = target_w * target_h * 3
    samples = ffi.new("unsigned char[?]", byte_count)
    ffi.fill(samples, byte_count, 255)
    pix = W.mupdf_new_pixmap_with_data(
        ctx, W.fz_device_rgb(ctx), target_w, target_h, nil, 0,
        target_w * 3, samples)
    if pix == nil then fail("cannot allocate RGB cover bitmap") end
    ...
    file:write(ffi.string(samples, byte_count))
    file:close()
end, debug.traceback)
