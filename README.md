...
- **Paperwhite 4（第 10 代，kindlehf 平台）** ✅
- **PW2 / PW3 / Voyage / Oasis 等 ARMv7 老机型** ✅（0.2.0 起验证）

覆盖原理：包内同时内置 `armv7` 与 `kindlehf` 两套裁剪运行时，启动时自动试跑并选用可用的一套；若设备装有 KOReader，也会作为兜底借用（非必需）。以后的新代际机型按同样方式加入对应运行时即可。
