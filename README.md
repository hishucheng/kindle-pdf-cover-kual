...
- **可强制刷新**：如果原地替换了 PDF、第一页发生变化，可在“高级工具 → 强制重建全部封面”中重新生成
- **纯本地**：无网络请求、无常驻进程、无定时任务、不额外耗电
- **不动书**：不修改 PDF 文件本身，不动阅读进度和账号数据
- **安全设计**：扫描前自动备份书库数据库；仅保留最近 10 份备份；写入前检查表结构，不兼容就停下；数据库被占用时不抢锁，直接放弃
...
- **Kindle 7（KT2，最终固件 5.12.2.2）** ✅ 实机验证（自动启用旧 Home 兼容模式）
...
Kindle 7 的旧版 Home 有一个固件级特殊行为：当书库记录的 MIME 为 `application/pdf` 时，会忽略已经存在的缩略图。插件会在该固件上自动生成 168×259 RGB 封面，并仅对本地 PDF 条目使用已实机验证的兼容 MIME；PDF 仍可正常打开。有新增封面或类型调整时，插件会在写库完成后自动重载 Home（不重启整机）；书库界面短暂刷新后即可显示。卸载前如需恢复原始 MIME，可运行“高级工具 → 恢复 PDF 类型标记”。
...
> 首次使用建议先跑一次"高级工具 → 兼容性检查"，确认环境没问题（结果会显示实际使用的运行时）。日常使用只需要第 3 步那一个按钮；只有 PDF 第一页已经变化、但原封面仍存在时，才需要“强制重建全部封面”。
...
5. 每轮扫描前自动把 `cc.db` 备份到 `extensions/pdf-cover/data/backups/`，并自动清理旧备份，仅保留最近 10 份
...
- 插件脚本：AGPL-3.0
- 渲染运行时提取自 KOReader 2026.07.1 的 Kindle 构建（armv7 / kindlehf），包含 MuPDF、LuaJIT、FreeType 等组件，仅用于渲染 PDF 首页；完整许可、精确上游版本与源码获取方式见 `LICENSE` 与 `THIRD_PARTY.md`

## 版本历史

- **5.0**：支持 Kindle 7 / 5.12.2.2 旧 Home；加入 RGB 168×259 渲染、PDF MIME 兼容、自动重载 Home 与一键恢复；机型检测兼容 prettyversion/version 双来源
